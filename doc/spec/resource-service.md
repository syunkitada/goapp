...
- IP ごとに専用 netns と vm-netns 間、netns-host 間をつなぐ bridge を作成する
  - netns 管理の IP として、192.168.0.0/19(192.168.0.1/192.168.15.254) を確保して利用する
  - 192.168.192.0/18 16384
  - 1 ノードで最大 2000 の VM を管理することを想定する
  - 一つの VM につき最低 5IP が必要
    - 4IP: hostnet との接続に利用: 192.168.192.0/19(8182)から割り当てる
    - 1IP: share-netns との接続に利用: 192.168.248.0/21(2048)の 192.168.248.40 以降から割り当てる
      - 192.168.248.1 - 192.168.248.40 は share-netns で利用する予約済み IP で、VM へのサービス用 IP として利用する
        - パッケージレポジトリサービス: http://192.168.248.2:8000
        - メタデータサービス: https://192.168.248.2:443
  - vm の ip は、host が router に bgp で広報し、外部からの通信は L3 通信により vm まで転送される
- 例
  - share(ip: 192.168.248.2) - bridge(192.168.248.1) netns - vm1-netns(192.168.248.41), vm2-netns(192.168.248.42)
