...
func (api *Api) GetUserAuthority(tctx *logger.TraceContext, db *gorm.DB, username string) (*spec.UserAuthority, error) {
	var err error
	startTime := logger.StartTrace(tctx)
	defer func() { logger.EndTrace(tctx, startTime, err, 1) }()

	var users []db_model.CustomUser
	query := "SELECT u.name, " +
	...
	userAuthority := spec.UserAuthority{
